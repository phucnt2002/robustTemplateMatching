...
if __name__ == '__main__':
    ...
    image_path = "sample/sample"
    template_path = "template/template"
    for index in range(8, 9):
        raw_image = cv2.imread(image_path+str(index)+".jpg")[..., ::-1]
        image = image_transform(raw_image.copy()).unsqueeze(0)

        raw_template = cv2.imread(template_path+str(index)+".jpg")[..., ::-1]
        ...
        boxes, centers, scores = FE(
            template, image, 0.95, True, index)
        d_img = raw_image.astype(np.uint8).copy()
        
        # Avoid index error if no box is detected
        if len(boxes) > 0:
            nms_res = nms(np.array(boxes), np.array(scores), thresh=0.05)
        ...
        if cv2.imwrite("./results/result"+str(index)+".jpg", d_img[..., ::-1]):
            print("./results/result"+str(index)+".jpg"+" was generated")
